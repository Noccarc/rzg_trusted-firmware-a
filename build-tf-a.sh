#!/bin/bash
BL2_PATH=build/g2l/release/
echo -e "\n[Creating bootparams.bin]"
SIZE=$(stat -L --printf="%s" ${BL2_PATH}/bl2.bin)
SIZE_ALIGNED=$(expr $SIZE + 3)
SIZE_ALIGNED2=$((SIZE_ALIGNED & 0xFFFFFFFC))
SIZE_HEX=$(printf '%08x\n' $SIZE_ALIGNED2)
echo " ${BL2_PATH}/bl2.bin size=$SIZE, Aligned size=$SIZE_ALIGNED2 (0x${SIZE_HEX})"
...
if [ "${BL2_PATH}/bl2.bin" -nt "bl2_bp.bin" ] || [p! -e "bl2_bp.bin" ] ; then
	echo -e "\n[Adding bootparams.bin to ${BL2_PATH}/bl2.bin]"
	cat bootparams.bin ${BL2_PATH}/bl2.bin > bl2_bp.bin
fi
